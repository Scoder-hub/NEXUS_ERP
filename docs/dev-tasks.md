...
| TASK-011 | 设备管理 — 台账 + 保养 + 维修           | 📝 待审批                       | 2026-07-11 |
...
**完成内容**:
- 编写了设备管理 PRD（TASK-011）
- 24 项功能需求（E-01 ~ E-24）
- 覆盖：设备台账、保养管理、维修管理、备件管理四大子模块
- 18 条业务规则（BR-E01 ~ BR-E18）
- 6 种异常流程、7 项边界条件、15 条验收标准
- 完整数据模型概要（6 张核心表：设备 / 保养计划 / 保养记录 / 维修工单 / 备件目录 / 备件出入库）
- 设备与工序对应关系表（8 道工序 × 主要设备）
- 关键设备清单（隧道窑、耐压机、球磨机等，含管理级别和特殊要求）
- 设备状态流转图（新增→运行→维修中/保养中/停机→已报废）
- 分阶段建议（档案+保养+维修全流程优先，状态管理/预警增强次之）

**关键决策**:
- 设备编号统一格式 `EQ-{工序缩写}-{3位流水}`，全局唯一
- 关键设备（窑炉、耐压机）保养记录不可删除
- 维修期间设备状态自动变更，维修完成后自动恢复
- 维修更换备件时联动备件库存自动减少
- 设备档案/保养/维修可独立先行开发，不依赖其他模块
- 备件管理依赖 TASK-008/009 的库存基础

**交接摘要（PM → UX设计师 + 架构师）**:

**下游角色需关注**:
- UX 设计师需设计：设备档案创建/编辑/详情页、保养计划配置页、保养执行页、维修报修/派工/执行/验收全流程页、备件目录/出入库页
- 架构师需定义：6 张新表（equipment / maintenance_plans / maintenance_records / repair_orders / spare_parts / spare_part_transactions）、15+ 个 IPC 通道、与 TASK-008/009 备件库存联动接口、设备状态变更日志

**关键决策**:
- 设备编号格式统一 `EQ-{工序缩写}-{3位流水}`
- 关键设备保养记录不可删除
- 维修状态自动变更，联动设备状态
- 设备档案/保养/维修可独立先行开发

**风险提示**:
- 设备管理大部分功能可独立先行开发，不依赖其他模块
- 备件管理与库存模块联动，建议等 TASK-008/009 就绪后集成
- 设备状态变更日志需要完整的审计链，数据量可能会较大
